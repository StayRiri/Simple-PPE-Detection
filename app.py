...
# Muat model ke memori cache (Penting untuk WebRTC)
@st.cache_resource
def load_model():
...
sumber_opsi = st.sidebar.radio("Pilih Sumber Input:", ["Webcam (Live Cloud)", "Upload Video"])
...
# ==========================================
# MODE 1: WEBCAM (VIA WEBRTC STREAMLIT)
# ==========================================
if sumber_opsi == "Webcam (Live Cloud)":
    with col1:
        st.subheader("📷 Visualisasi Deteksi WebRTC")
        st.info("💡 Tekan tombol 'START' untuk meminta izin kamera browser. Tabel log akan terupdate setelah Anda menekan tombol Refresh.")
        
        # Variabel global khusus untuk menyimpan memori tracking di thread terpisah
        webrtc_pemetaan_id = {}
        webrtc_id_urut = 1
        webrtc_pelanggar = set()

        def video_frame_callback(frame: av.VideoFrame) -> av.VideoFrame:
            global webrtc_id_urut
            ...
            boxes_person, boxes_helmet, boxes_vest, boxes_gloves, boxes_boots = [], [], [], [], []
            ...
            for result in results:
                ...
                for box in result.boxes:
                    ...
                    if class_name == 'Person' and track_id != -1:
                        if track_id not in webrtc_pemetaan_id:
                            webrtc_pemetaan_id[track_id] = webrtc_id_urut
                            webrtc_id_urut += 1
                        id_final = webrtc_pemetaan_id[track_id]
                        label_teks = f"Pekerja ID:{id_final}"
                    else:
                        label_teks = f"{class_name}"
                        
                    cv2.rectangle(img, (x1, y1), (x2, y2), warna, 2)
                    cv2.putText(img, label_teks, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, warna, 2)
            ...
            for px1, py1, px2, py2, track_id in boxes_person:
                ...
                def catat_pelanggaran(kondisi_pakai, nama_pelanggaran):
                    if not kondisi_pakai:
                        ...
                        if kunci_memori not in webrtc_pelanggar:
                            log_baru.append({
                                "Waktu": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 
                                "ID Pekerja": f"Pekerja #{track_id}",
                                "Jenis Pelanggaran": nama_pelanggaran, 
                                "Lokasi": "Zona Kamera Web"
                            })
                            webrtc_pelanggar.add(kunci_memori)
        ...
        # Tombol manual untuk refresh tabel (karena WebRTC berjalan di thread terpisah)
        if st.button("🔄 Refresh Tabel Log"):
            table_placeholder.dataframe(load_logs(), use_container_width=True, hide_index=True)


# ==========================================
# MODE 2: UPLOAD VIDEO LOKAL
# ==========================================
elif sumber_opsi == "Upload Video":
    with col1:
        st.subheader("Visualisasi Deteksi Upload")
        ...
        try:
            if file_video is None:
                ...
                while cap.isOpened() and not stop_button:
                    ret, frame = cap.read()
                    if not ret:
                        st.info("Pemutaran video selesai.")
                        break
